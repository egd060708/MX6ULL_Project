...
unsigned short lcd_read_panelid(void)
{
	...
	/* 配置模拟开关IO */
	gpio_init_config_s idio_config = {
		.gpio_group = GPIO3,
		.pin_num = 3,
		.inputOnfield = 0,
		.configValue = 0X10B0,
	...
	gpio_init(IOMUXC_LCD_VSYNC_GPIO3_IO03, &idio_config);

	/* 读取ID值，设置G7 B7 R7为输入 */
	idio_config.pin_num = 12;
	idio_config.configValue = 0xF080;
	idio_config.direction = kGPIO_DigitalInput;
	gpio_init(IOMUXC_LCD_DATA07_GPIO3_IO12, &idio_config);
	idio_config.pin_num = 20;
	gpio_init(IOMUXC_LCD_DATA15_GPIO3_IO20, &idio_config);
	idio_config.pin_num = 28;
	gpio_init(IOMUXC_LCD_DATA23_GPIO3_IO28, &idio_config);
...
void lcdgpio_init(void)
{
	...
	gpio_init(IOMUXC_GPIO1_IO08_GPIO1_IO08, &gpio_config);	/* 背光默认打开 */
	gpio_pin_write(GPIO1, 8, 1);								/* 打开背光     */
}
